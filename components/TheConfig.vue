...
<template>
  <div
      :class="[
          ...
          isExpanded && 'p-4 bg-[var(--color-bg)] text-[var(--color-text)] border-[var(--color-border)] shadow-[0_4px_6px_var(--color-shadow)] rounded-lg'
      ]"
  >
    <div class="flex justify-between w-full">
      <!-- Toggle Light/Dark Mode Button (Visible only when expanded) -->
      <button
          v-if="isExpanded && themes[activeTheme]?.light && themes[activeTheme]?.dark"
          @click="toggleMode"
      ...
      >
        <i
            :class="[
              'text-md',
              activeMode === 'light' ? 'i-tabler-moon' : 'i-tabler-sun',
          ]"
        ></i>
      </button>
      ...
      >
        <i :class="[
          'text-xl',
          isExpanded ? 'i-tabler-arrow-right' : 'i-tabler-arrow-left',
      ]"></i>
